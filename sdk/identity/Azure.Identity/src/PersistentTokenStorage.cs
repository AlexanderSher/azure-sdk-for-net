...
namespace Azure.Identity
{
    ...
    public class PersistentTokenStorage : TokenStorage
    {
        ...
        private static AsyncLockWithValue<MsalCacheHelper> s_cacheHelperLock = new AsyncLockWithValue<MsalCacheHelper>();
        ...
        public PersistentTokenStorage(bool allowUnencryptedStorage = true)
        {
            _allowUnencryptedStorage = allowUnencryptedStorage;
        }

        /// <summary>
        /// Creates a new instance of <see cref="PersistentTokenStorage"/> with the specified options.
        /// </summary>
        /// <param name="options">Options controlling the storage of the <see cref="PersistentTokenStorage"/>.</param>
        public PersistentTokenStorage(PersistentTokenStorageOptions options)
        {
        ...
        /// <summary>
        /// Registers storage for cache
        /// </summary>
        /// <param name="tokenCache"></param>
        /// <param name="cancellationToken"></param>
        public override void Register(TokenCache tokenCache, CancellationToken cancellationToken)
        {
            MsalCacheHelper cacheHelper = GetCacheHelperAsync(false, cancellationToken).EnsureCompleted();
            cacheHelper.RegisterCache(tokenCache.Cache);
        }
        ...
        {
            MsalCacheHelper cacheHelper = await GetCacheHelperAsync(true, cancellationToken).ConfigureAwait(false);
            cacheHelper.RegisterCache(tokenCache.Cache);
        }

        private async Task<MsalCacheHelper> GetCacheHelperAsync(bool async, CancellationToken cancellationToken)
        {
            using var asyncLock = await s_cacheHelperLock.GetLockOrValueAsync(async, cancellationToken).ConfigureAwait(false);
